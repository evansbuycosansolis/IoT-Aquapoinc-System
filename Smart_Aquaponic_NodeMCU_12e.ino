...
const char* servername = "http://ua-tlmc-laboratory-management-system-with-sms-notification.site/get_sensor_state.php";
...
if(WiFi.status()== WL_CONNECTED){
  if (espSerial.available()) 
  {
    ...
    http.end();
                                                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                                                        if(pHValue>9){digitalWrite(output5, HIGH);} else {digitalWrite(output5, LOW);}
                                                                                                                                                                                                                                                                                        if(hardness>300){digitalWrite(output5, HIGH);} else {digitalWrite(output5, LOW);}
                                                                                                                                                                                                                                                                                        if(disslved_o>10){digitalWrite(output4, HIGH);}else {digitalWrite(output4, LOW);}
   }
  ...
  if(currentMillis - previousMillisx >= intervalx) {
    ...
    if(WiFi.status()== WL_CONNECTED ){ 
      ...
      for (int i = 0; i < keys.length(); i++) {
        JSONVar value = myObject[keys[ph]];
        Serial.print("GPIO: ");
        Serial.print(keys[i]);
        Serial.print(" - SET to: ");
